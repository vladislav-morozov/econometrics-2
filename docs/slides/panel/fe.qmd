...
First consider one-way model $Y_{it} = \alpha_{i} + \bX_{it}'\bbeta + U_{it}$. For $\bW_{it} = Y_{it}, \bX_{it}, U_{it}$, define the (one-way) <span class="highlight">within-transformed</span> version of $W_{it}$ as
$$ \small
...
- Approach problem differently:
  - First formulate estimator; only after study causal properties
  - Usually goes the other way: fix causal problem, figure out identification and estimation
- Doing so for historic reasons — estimators came first, serious causal thinking more recently 
...
Realized data satisfies $\tilde{Y}_{it} = \tilde{\bX}_{it}'\bbeta + \tilde{U}_{it}$ 
