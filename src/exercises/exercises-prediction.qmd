...
As [noted](https://vladislav-morozov.github.io/econometrics-2/slides/prediction/regression-pt2.html#/bias-variance-decomposition-ii) in the second lecture on predictive regression, one can establish a fairly explicit decomposition for the expected mean squared error (MSE) of a hypotheses picked on a random sample. 



In this problem, you are asked to prove this bias-variance decomposition  (equation (1) in the [slides](https://vladislav-morozov.github.io/econometrics-2/slides/prediction/regression-pt2.html#/bias-variance-decomposition-ii)). Define the following objects:
...
& \quad + \E_S\left[ \left(\E_{Y|\bX}[Y|\bX=\bx] - \E_S[\hat{h}_S(\bx)] \right)\left( \E_S[\hat{h}_S(\bx)] - \hat{h}_S(\bx) \right)    \right] .
\end{aligned}
$$
...
\begin{aligned}
& \E_S\left[MSE(\hat{h}_S) \right] \\
& = \E_S\left[ \E_{Y, \bX}[U^2]\right] -2 \E_{S}\left[\E_{Y, \bX}\left[U(\E[Y|\bX] - \hat{h}_S(\bX))\right] \right] \\
...
$$
\begin{aligned}
& \E_{S}\left[\E_{Y, \bX}\left[U(\E[Y|\bX] - \hat{h}_S(\bX))\right] \right]\\
& = \E_{S}\left[\E_{\bX}\left[   (\E[Y|\bX] - \hat{h}_S(\bX))
