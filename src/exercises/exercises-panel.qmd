...
1. Recall that in a linear causal model OLS is consistent for the coefficient vector provided the regressors $\bX_{it}$ are orthogonal to the residuals in the sense $\E[\bX_{it}U_{it}]=0$. Does this condition hold in the above regression? What about $\E[U_{it}|\bX_{it}]=0$? Connect to the assumption of no trends.
