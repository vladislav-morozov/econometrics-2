...
Can estimate $V$ with 
$$
\hat{V} = \widehat{\var}(Z_i) = \dfrac{1}{N}\sum_{i=1}^N\left(Z_i - \dfrac{1}{N}\sum_{j=1}^N Z_j \right)^2
$$
...
Can now construct confidence intervals and hypothesis tests about $\E[Y_{i \tau}^1 - Y_{i\tau}^0]$. E.g. an asymptotic (1-\alpha)\times 100% confidence interval:
$$
\widehat{CI}_{95\%} = \left[\hat{\beta}_{\tau} - z_{1-\alpha/2}\widehat{se}(\hat{\beta}_{\tau}), \hat{\beta}_{\tau} + z_{1-\alpha/2}\widehat{se}(\hat{\beta}_{\tau}) \right]
$$
...
# To download the tickers and rename columns to only retain ticker names run the following:
...
# We load in the data from a CSV file to avoid repeated downloads
# to ensure reproducibility and avoid hitting API limits
stock_data_path = Path() / 'data' / 'banks_tickers.csv'
