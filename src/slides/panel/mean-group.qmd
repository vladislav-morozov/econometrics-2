...
- $\bbeta_i$ — $p$-vector
- Assume that $T\geq p$ (at least as many observations as there are coefficients)
- Interested in $\E[\bbeta_i]$
...
- We assume that $(\bX_i'\bX_i)^{-1}$ exists
  ...
  - May be tougher to satisfy if $T$ is small and and $\bX_{it}$ has discrete regressors
...
- Before assumed that all units had invertible $(\bX_i'\bX_i)$
...
Need $\avar(\hat{\bbeta}^{MG})$ if we want to do inference on $\E[\bbeta_i]$
