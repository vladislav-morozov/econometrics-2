...
This lecture is about a way to estimate an average of heterogeneous coefficients
...
Step (1) not part of MG estimation, needed here due to our model
...
First approach more obvious, so will do second
...
ax.set_xlim([-150, 100])
ax.legend()
ax.set_title('Density of individual estimators', color=FONT_COLOR, loc='left')
ax.set_xlabel('')
plt.show()
...
Variance $\var(\beta_i)$. can be identified and estimated, see [some of my notes](https://vladislav-morozov.github.io/econometrics-heterogeneity/linear/linear-coefficient-variance.html)
...
fig.update_layout( 
    ...
    title=dict(
        text="<b>Effect of an additional smoke day on quarterly earnings (Q3)</b>",
        x=0.03,
        xanchor="left",
        y=0.97,
        yanchor="top",
