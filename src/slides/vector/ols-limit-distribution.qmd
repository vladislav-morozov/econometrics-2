...
This lecture is about the asymptotic distribution of the OLS estimator and why we need it
...
- Provide definition of convergence in distribution
- State the multivariate central limit theorem along with Slutsky's theorem
...
$$
where $\Phi(\cdot)$ is the standard normal. In other words, the mean is approximately distrubuted as ("$\overset{a}{\sim}$")
$$\small
\bar{X} \overset{a}{\sim} N\left( \E[X_i], \dfrac{\var(X_i)}{N} \right)
...
Let $\bX_N\xrightarrow{d}\bX$, $\bY_N\xrightarrow{p} \by$, $\bA_N\xrightarrow{p}\bA$. Then (provided invertibility and compatible sizes) 

1. $\bX_N + \bY_N \xrightarrow{d} \bX + \by$
2. $\bA_N\bX_N \xrightarrow{d} \bA\bX$
3. $\bX_N'\bA_N^{-1}\bX_N\xrightarrow{d} \bX'\bA^{-1}\bX$
...
#### Warning: $\by$ and $\bA$ Must Be Constant

<br>

::: {.callout-important}

Slutsky's theorem no longer holds if $\bY_N$ or $\bA_N$ converges to a random limit! 
...
Then
$$\small
\sqrt{N}(\hat{\bbeta}-\bbeta) \xrightarrow{d} N\left(0, \left( \E[\bX_i\bX_i']\right)^{-1} \E[U_i^2\bX_i\bX_i']\left( \E[\bX_i\bX_i']\right)^{-1}  \right)
$$
...
- If $\E[U_i^2|\bX_i]=\sigma^2$ (conditional homoskedasticity), expression simplifies. In practice not very useful (corresponds to `nonrobust` estimates of standard errors)
...
- If a hypothesis tells us something about $\bbeta$, we can check how likely based on seeing  $\hat{\bbeta}$
