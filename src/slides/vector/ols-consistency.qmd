...
## Consistency of the OLS Estimator: Model Free {background="#00100F"}
...
1. "Model-free": no causal model assumed, behavior of $\hat{\bbeta}$ in terms of some moments of $\bX$ and $\bY$
2. Causal model with <span class="highlight">homogeneous</span> effects: there a true effect $\bbeta$, same for everyone
3. Causal model with <span class="highlight">heterogeneous</span> effects: each unit $i$ has their own true effect $\bbeta_i$ (but we still run only one OLS procedure)
...
## Consistency of the OLS Estimator: Causal Settings {background="#00100F"}
