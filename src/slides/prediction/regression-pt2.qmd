...
- Can specialize it a bit more for MSE — the <span class="highlight">bias-variance</span> trade-off
...
\begin{aligned}
MSE(h)  & = \E[(Y-h(\bX))^2] \\ 
& = \sigma^2  + \E\left[ (\E[Y|\bX]- h(\bX))^2 \right] + \var(h(\bX))
\end{aligned}
$$
 
::: {.footer}

To show the decomposition, proceed with MSE as before. Wikipedia offers [a full derivation](https://en.wikipedia.org/wiki/Bias%E2%80%93variance_tradeoff#Derivation)
...
The bias interpretation comes from writing $\E\left[ (\E[Y|\bX]- h(\bX))^2 \right]=  \E\left[ \E\left[  (\E[Y|\bX]- h(\bX))^2 |\bX \right]\right]$. Inside the conditional the terms are like fixed parameters
