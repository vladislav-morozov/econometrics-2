...
- On step $j$, the $j$th fold (orange) excluded from training
...
- Number $k$ of folds — trade-off between computational easy and estimation quality for risk (more on that later)
...
```{python}
#| echo: true
pd.Series(ols_rmse).describe().iloc[]
```
...
- Story so far unclear on the virtues of nonlinearis in $\bX$
