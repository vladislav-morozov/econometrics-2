...
- Split $S$ sample into two sets: 
  - Training set $S_{Tr}$ used for selecting some $\hat{h}_{S_{Tr}}$
...
We will use cross-validation, no need to explicitly split validation set
...
fig = px.scatter_geo(
    ...
    scope="usa",
    opacity=0.4,
    color=data_df["MedHouseVal"],
    color_continuous_scale="RdBu_r", # _r reverses the colorscale
    size = data_df['Population'],
)

# automatically zoom on the data
...
train_set.hist(ax = ax, bins=50, grid=False);
...
Prisons are the source of these issues. A flexible model (like a tree/forest) can learn the pattern "high `AveOccup` = prison" and adjust if there is enough data. Less flexible models might suffer
...
- Some probably not directly helpful (longitude and latitude), though maybe can be transformed into something more useful
...
## Model Development I: Linear Regression {background="#00100F"}
...
By proceeding as in @prp-pred-mse-opt (check!) can write 
$$
\begin{aligned}
MSE(h)  & = \E[(Y-h(\bX))^2] \\
...
### Cross-Validation and Penalized Approaches {background="#43464B" visibility="uncounted"}
