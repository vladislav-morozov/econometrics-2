...
- It will use the <span class="highlight">same</span> parameters (also with validation and test data)
...
- Fully custom ones, just need to implement minimal (inheriting from `TransformerMixin` is helpful)
...
- Can operate a single transformer
...
Only then make polynomials (maybe) and standardize
...
- Note: adds component names ot columns names: two underscores `__` separate component names
- Columns not specified handled according to `remainder` argument
...
- Pipeline ingests original data, applies all our steps, and gives a preprocessed data
```{python}
#| echo: true
pd.DataFrame(
