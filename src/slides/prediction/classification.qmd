...
- By default $\theta = 0.5$ — label observation as 1 if predicted probability $\Lambda( \varphi(\bX)'\hat{\bbeta}  )$
- But can change $\theta$. E.g. lowering $\theta$:
  - Will predict positive class more often (more true positives)
  - But will also have more false positives
...
- In practice:
  - First compute the <span class="highlight">scores</span> $\Lambda(\bX_i'\hat{\bbeta})$ for each observation  
  - Then $\theta$, compute which observations labeled as positive or negative  by comparing  $\Lambda(\bX_i'\hat{\bbeta})$ and $\theta$
...
- For $\theta\uparrow 1$ both recall and precision fall
