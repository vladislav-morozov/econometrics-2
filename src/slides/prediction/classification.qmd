...
```{python}
#| echo: true
y_train_pred = cross_val_predict(logit, X_train, y_train_0, cv=5, n_jobs=-1)
...
```{python}
#| echo: true
print(f"Precision: {np.round(precision_score(y_train_0, y_train_pred), 3)}")
print(f"Recall: {np.round(recall_score(y_train_0, y_train_pred), 3)}")
```
Very close 
...
```{python}
print(f"F1: {np.round(f1_score(y_train_0, y_train_pred), 2)}")
```
